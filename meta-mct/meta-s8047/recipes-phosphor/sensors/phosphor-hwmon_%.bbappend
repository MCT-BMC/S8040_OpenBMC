FILESEXTRAPATHS_prepend_s8047 := "${THISDIR}/${PN}:"
EXTRA_OEMESON_append_s8047 = " -Dupdate-functional-on-fail=false -Dnegative-errno-on-fail=false"

SRC_URI += "file://0001-Add-power-on-monitor-mechanism.patch \
	   "

# Shale specific sensors
CHIPS_SHALE = " \
...
CHIPS_CINNABAR = " \
        bus@1e78a000/i2c-bus@180/nct7802@28 \
        bus@1e78a000/i2c-bus@280/mp2856@61 \
        bus@1e78a000/i2c-bus@280/mp2856@62 \
...
ITEMS_SHALE = "${@compose_list(d, 'ITEMSFMT', 'CHIPS_SHALE')}"
ITEMS_SHALE += "iio-hwmon-adc121c.conf"
ITEMS_CINNABAR = "${@compose_list(d, 'ITEMSFMT', 'CHIPS_CINNABAR')}"
ITEMS_CINNABAR += "iio-hwmon.conf"
ITEMS_SUNSTONE = "${@compose_list(d, 'ITEMSFMT', 'CHIPS_SUNSTONE')}"
ITEMS_SUNSTONE += "iio-hwmon-adc121c.conf"

ENVS_SHALE64 = "obmc/hwmon_shale/{0}"
SYSTEMD_ENVIRONMENT_FILE_${PN}_append_s8047 += " ${@compose_list(d, 'ENVS_SHALE64', 'ITEMS_SHALE')}"
ENVS_CINNABAR = "obmc/hwmon_cinnabar/{0}"
SYSTEMD_ENVIRONMENT_FILE_${PN}_append_s8047 += " ${@compose_list(d, 'ENVS_CINNABAR', 'ITEMS_CINNABAR')}"
ENVS_SUNSTONE = "obmc/hwmon_sunstone/{0}"
SYSTEMD_ENVIRONMENT_FILE_${PN}_append_s8047 += " ${@compose_list(d, 'ENVS_SUNSTONE', 'ITEMS_SUNSTONE')}"
